...
- GitHub CLI is available as `gh`.
- The GitHub account is already authenticated.
- You and Corey use the same authenticated GitHub account for repository access.
- You may use `gh` and `git` for normal repository read and clone operations needed for assigned work.
...
- Work across repositories from this root when issues span the system.
- Keep repo-local instructions and rules in sync with task execution.
- Coordinate implementation handoff to Mark through Paperclip issues.
